...
import com.tricheer.launcherg.adapter.display.OnScreenIDFragItemAdapter;
...
/**
 * [Menu-Display-On-screen ID] Fragment
 *
 * @author Jun.Wang
 */
public class OnScreenIDFragment extends BaseFragment {
    // TAG
    private static final String TAG = "OnScreenIDFragment";
    ...
    //Variables
    private OnScreenIDActivity mAttachedActivity;
    private OnScreenIDFragItemAdapter mLvItemAdapter;

    @Override
    public void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        mAttachedActivity = (OnScreenIDActivity) getActivity();
    }
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        fragV = inflater.inflate(R.layout.frag_display_onscreenid, null);
        return fragV;
    }
    ...
    private void loadLvContent() {
        // Construct data list
        List<OnScreenIDFragItem> listItems = new ArrayList<OnScreenIDFragItem>();

        OnScreenIDFragItem item01 = new OnScreenIDFragItem();
        item01.name = mAttachedActivity.getString(R.string.on_upper);
        item01.setTag(AppComManager.ON);
        listItems.add(item01);

        OnScreenIDFragItem item02 = new OnScreenIDFragItem();
        item02.name = mAttachedActivity.getString(R.string.off_upper);
        item02.setTag(AppComManager.ON);
        listItems.add(item02);

        // Load ListView
        mLvItemAdapter = new OnScreenIDFragItemAdapter(mAttachedActivity, 0);
    ...
    public void loadPageByIdx(int pos) {
        ...
        if (pos != getSelectedPosByTag()) {
            ...
            PreferUtils.isShowOnScreenID(true, isShow);
            AppStackManager.notifyRefreshOnScreenID(isShow);
        }
    }
    ...
    private int getSelectedPosByTag() {
        return PreferUtils.isShowOnScreenID(false, false) ? 0 : 1;
    }
}